...
public class GestureClassification {
    ...
    protected static int numExamples = 800;
    protected static int numLabels = 5;
    protected static int batchSize = 160;
    ...
    public void run(String[] args) throws Exception {
        ...
        File mainPath = new File(System.getProperty("user.dir"), "src/main/resources/Photos/");
        ...
        List<String> labels = extractLabels(testData);
...
//
        ...
        List<String> labelNamesList = testDataSet.getLabelNamesList();

        for(int i=0;i<labels.size();i++) {
            System.out.print("\nFor a single example that is labeled " + labels.get(i) + " the model predicted " + predict.get(i) + "\n\n");
        }
    ...
    private void shuffle(InputSplit data) {
        URI[] locations = data.locations();
        Random rnd = ThreadLocalRandom.current();
        for (int i = locations.length - 1; i > 0; i--)
        {
            int index = rnd.nextInt(i + 1);
            // Simple swap
            URI a = locations[index];
            locations[index] = locations[i];
            locations[i] = a;
        }
    }

    private List<String> extractLabels(InputSplit testData) {
        List<String> list = new ArrayList<>();
        for (URI uri : testData.locations()) {
            String ur = uri.toString();
            if(ur.contains("fists")) {
                list.add("fists");
            } else if(ur.contains("fuck_you")) {
                list.add("fuck_you");
            } else if(ur.contains("palms")) {
                list.add("palms");
            } else if(ur.contains("thumbs")) {
                list.add("thumbs");
            } else {
                list.add("v");
            }
